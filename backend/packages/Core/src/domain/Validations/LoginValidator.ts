...
export class LoginValidator {
  ...
  Validate(): ValidationError<Credentials>[] {
    ...
    if (
      !this.credentials.serverInformation.server ||
      this.credentials.serverInformation.server.trim() === ''
    ) {
      errors.push({ field: 'serverInformation', message: 'Server is required' });
    }

    if (
      !this.credentials.serverInformation.apiKey ||
      this.credentials.serverInformation.apiKey.trim() === ''
    ) {
      errors.push({ field: 'serverInformation', message: 'ApiKey is required' });
    }

    if (
      !this.credentials.serverInformation.apiSecret ||
      this.credentials.serverInformation.apiSecret.trim() === ''
    ) {
      errors.push({ field: 'serverInformation', message: 'ApiSecret is required' });
    }

    if (
      !this.credentials.serverInformation.organization ||
      this.credentials.serverInformation.organization.trim() === ''
    ) {
      if (
        !this.credentials.serverInformation.dataBase ||
        this.credentials.serverInformation.dataBase.trim() === ''
      ) {
