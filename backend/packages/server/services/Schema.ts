...
export class SchemaService extends BaseService {
   ...
   */
  private async executeUseCase<T>(
    useCase: UseCase<T>,
  ): Promise<CoreResult<T, AppCodeError, Error>> {
    try {
  ...
  async getDocumentTypeSchema(
    ...
    const request = this.buildRequest<GetDocumentTypeSchemaRequest>({
      credentials,
      documentTypeId,
    });
  ...
  async getSystemDataElements(
    credentials: Credentials,
  ): Promise<CoreResult<Array<DataElement>, AppCodeError, Error>> {
    const request = this.buildRequest<Credentials>(credentials);
  ...
  async createDocumentGroup(
    body: CreateDocumentGroupRequest,
  ): Promise<CoreResult<DocumentGroup, AppCodeError, Error>> {
  ...
  async createDataElement(
    body: CreateDataElementRequest,
  ): Promise<CoreResult<DataElement, AppCodeError, Error>> {
